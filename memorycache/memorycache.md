# MemoryCache
MemoryCache is an LRU Cache, written in Go, focused on supporting high concurrency.
...
First, download the project:
    ...
    go get github.com/xpsuper/stl/memorycache
...
var cache = memorycache.New(memorycache.Configure())
```

`Configure` exposes a chainable API:

```go
var cache = memorycache.New(memorycache.Configure().MaxSize(1000).ItemsToPrune(100))
```
...
* `Buckets` - memorycache shards its internal map to provide a greater amount of concurrency. Must be a power of 2 (default: 16).
...
By returning expired items, memorycache lets you decide if you want to serve stale content or not. For example, you might decide to serve up slightly stale content (< 30 seconds old) while re-fetching newer data in the background. You might also decide to serve up infinitely stale content if you're unable to get new data from your source.
...
## Tracking
memorycache supports a special tracking mode which is meant to be used in conjunction with other pieces of your code that maintains a long-lived reference to data.

When you configure your cache with `Track()`:

```go
cache = memorycache.New(memorycache.Configure().Track())
```
...
memorycache's `LayeredCache` stores and retrieves values by both a primary and secondary key. Deletion can happen against either the primary and secondary key, or the primary key only (removing all values that share the same primary key).
...
```go
cache := memorycache.Layered(memorycache.Configure())
...
However, if the values you set into the cache have a method `Size() int64`, this size will be used. Note that memorycache has an overhead of ~350 bytes per entry, which isn't taken into account. In other words, given a filled up cache, with `MaxSize(4096000)` and items that return a `Size() int64` of 2048, we can expect to find 2000 items (4096000/2048) taking a total space of 4796000 bytes.
