...
func (instance *XPConfigImpl) getEnvironmentPrefix(config interface{}) string {
	if instance.XPConfigEnvironment.EnvironmentPrefix == "" {
		if prefix := os.Getenv("XPCONFIG_ENV_PREFIX"); prefix != "" {
			return prefix
		}
	...
	return instance.XPConfigEnvironment.EnvironmentPrefix
}

func getConfigurationFileWithEnvironmentPrefix(file, env string) (string, error) {
	var (
		envFile string
...
func (instance *XPConfigImpl) getConfigurationFiles(files ...string) []string {
	var results []string
	...
	for i := len(files) - 1; i >= 0; i-- {
		...
		if file, err := getConfigurationFileWithEnvironmentPrefix(file, instance.GetEnvironment()); err == nil {
			foundFile = true
			results = append(results, file)
		}
		...
		if !foundFile {
			if example, err := getConfigurationFileWithEnvironmentPrefix(file, "example"); err == nil {
