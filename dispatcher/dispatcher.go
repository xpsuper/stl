...
// Dispatcher is responsible for scheduling jobs.
...
func NewDispatcher(cnt int) (*Dispatcher, error) {
	if cnt > maxJobSetSize {
		return nil, ErrOverlength
	}
	if cnt <= 0 {
		cnt = defaultJobSetSize
	}
...
func (instance *Dispatcher) Delay() *Job {
	instance.js.lock.Lock()
	defer instance.js.lock.Unlock()

	// temporarily handle like this
	if len(instance.js.pendingSet) >= 10000 {
	...
	instance.js.pendingSet[id] = j
	return j
}

// Every method schedule job with Every mode.
func (instance *Dispatcher) Every() *Job {
	instance.js.lock.Lock()
	defer instance.js.lock.Unlock()
...
func (instance *Dispatcher) PendingJob(id string) (*Job, error) {
	instance.js.lock.Lock()
	defer instance.js.lock.Unlock()

	if job, ok := instance.js.pendingSet[id]; ok {
		return job, nil
	}
...
func (instance *Dispatcher) JobType(id string) (string, error) {
	job, err := instance.PendingJob(id)
	if err != nil {
...
func (instance *Dispatcher) JobSched(id string) (map[string]int, error) {
	job, err := instance.PendingJob(id)
	if err != nil {
...
func (instance *Dispatcher) JobDone(id string) (bool, error) {
	instance.js.lock.Lock()
	defer instance.js.lock.Unlock()

	if _, ok := instance.js.completedSet[id]; ok {
		return true, nil
	}
	return false, nil
}
...
func (instance *Dispatcher) CancelJob(id string) error {
	// can not cancel a completed job
	if _, ok := instance.js.completedSet[id]; ok {
		return ErrAlreadyComplayed
	}
	// can not cancel a nonexistent job
	if _, ok := instance.js.pendingSet[id]; !ok {
		return ErrPendingJob
	}

	// cancel by job type
	job := instance.js.pendingSet[id]
	switch job.Type {
	case Delay:
