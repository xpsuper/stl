...
type User struct {
    // Basic string field
    Name  string
    // DeepCopy supports https://golang.org/pkg/database/sql/driver/#Valuer
...
type UserResource struct {
    DisplayName            string `deepcp:"field:Name"`
    SkipMe                 string `deepcp:"skip"`
    MethodThatTakesContext string `deepcp:"context"`
    Email                  string `deepcp:"force"`
}
...
func main() {
    ...
    stl.IMP.DeepCopy(user).To(resource)
...
const (
	// TagName is the deepcp struct tag name.
	TagName = "deepcp"
...
// deepcp deep copies a struct to/from a struct.
type XPDeepCPImpl struct {
	dst interface{}
	src interface{}
...
func DeepCopy(src interface{}) *XPDeepCPImpl {
	return &XPDeepCPImpl{src: src}
}

// WithContext injects the given context into the builder instance.
func (dc *XPDeepCPImpl) WithContext(ctx map[string]interface{}) *XPDeepCPImpl {
	dc.ctx = ctx
	return dc
}

// To sets the destination.
func (dc *XPDeepCPImpl) To(dst interface{}) error {
	dc.dst = dst
	return process(dc.dst, dc.src, CPOptions{Context: dc.ctx})
}

// From sets the given the source as destination and destination as source.
func (dc *XPDeepCPImpl) From(src interface{}) error {
	dc.dst = dc.src
	dc.src = src
	return process(dc.dst, dc.src, CPOptions{Context: dc.ctx, Reversed: true})
}

// process handles copy.
func process(dst interface{}, src interface{}, args ...CPOptions) error {
	var (
		options        = CPOptions{}
...
// getTagOptions parses deepcp tag field and returns options.
func getTagOptions(value string) TagOptions {
	...
	for _, opt := range strings.Split(value, ";") {
		...
		// deepcp:"keyword; without; value;"
		if len(o) == 1 {
			options[o[0]] = ""
		}

		// deepcp:"key:value; anotherkey:anothervalue"
		if len(o) == 2 {
